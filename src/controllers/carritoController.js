...
// Controlador para obtener el carrito de compra
...
async function agregarProductoCarrito(req, res) {
  ...
  try {
    const nuevoProductoCarrito = await CarroCompra.create({
      idusuario: idUsuario,
      idproducto: idProducto
    });

    res.status(201).json(nuevoProductoCarrito);
  } catch (error) {
    console.error(error);
...
async function eliminarProductoCarrito(req, res) {
  ...
  try {
    const productoCarrito = await CarroCompra.findByPk(id);
